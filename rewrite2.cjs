...
// Find start of mock data
const mockStart = lines.findIndex(l => l.includes('const initialMachines: Machine[] = ['));
// Find start of useMantePro
const useManteProStart = lines.findIndex(l => l.includes('export function useMantePro() {'));

if (mockStart > -1 && useManteProStart > -1) {
  const newMockDataAndProvider = `
...
export function MantePoProvider({ children }: { children: ReactNode }) {
  ...
  const addMachine = async (m: Omit<Machine, "id">) => { fetchAllData(); };
  ...
  const addRecord = (r: Omit<MaintenanceRecord, "id">) => {
    ...
    supabase.from('ordenes_trabajo').insert({
      numero_otm: \`OTM-S-\${n}\`, maquina_id: r.machineId, estado: r.status, fecha_programada: r.date
    }).then(() => fetchAllData());
    return \`OTM-S-\${n}\`;
  };
...
`;
  const finalLines = [...lines.slice(0, mockStart), newMockDataAndProvider, ...lines.slice(useManteProStart)];
  ...
  console.log("Rewrote successfully using slice.");
} else {
