...
const providerEndIndex = lines.findIndex(l => l.includes('return <Ctx.Provider value={value}>{children}</Ctx.Provider>;'));

// The provider ends at providerEndIndex + 1 (the closing bracket '}')
if (mockStart > -1 && providerEndIndex > -1) {
  const newCode = `
...
`;
  ...
  const finalLines = [...lines.slice(0, mockStart), newCode, ...lines.slice(providerEndIndex + 2)];
