...
if (!code.includes('getMachineUsagePct')) {
  // Inject before "export interface AppSettings"
  code = code.replace('export interface AppSettings', helpers + '\nexport interface AppSettings');
}

if (!code.includes('clearAllUsageLogs: () => void;')) {
  // Inject clearAllUsageLogs into State
  code = code.replace(/addUsageLog: \(log: Omit<UsageLog, "id">\) => void;\n/, 'addUsageLog: (log: Omit<UsageLog, "id">) => void;\n  clearAllUsageLogs: () => void;\n');
}

// Fix uuid error (J. Mendoza) in addUsageLog
...
// Fix syntax error in setUsageCycles mapping
...
const badUsageCycleMapRegex2 = /setUsageCycles\(ucData\.map\(\(u: any\) => \(\{\n          id: u\.id, machineId: u\.maquina_id, startAt: u\.start_at, horasAcumuladas: u\.horas_acumuladas, resetAt: u\.ultimo_reset\n        \}\)\)\);\n      \}/;
...
} else if (code.match(badUsageCycleMapRegex2)) {
  code = code.replace(badUsageCycleMapRegex2, correctUsageCycleMap);
}
